...
The COVID-19 pandemic profoundly impacted the City of Chicago, particularly communities of color on the South and West sides. As of October 20th 2021, COVID-19 has claimed 6024 lives, mostly in majority Black and Hispanic neighborhoods and among people over 60 years old. This analysis uses City of Chicago Department of Public Health data to generate a high-level over view of pandemic trends and assess the effectiveness of vaccine deployment in preventing death among the most vulnerable populations.
...
Cases spike dramatically at different times, while hospitalizations and deaths remain relatively more stable. Next, let's focus in on death rates by race.
<br/><br/>
```{r}
chi_covid_stats %>% 
...
Non-Black and non-Latinx races are shaded green to highlight the racially disproportionate impacts of the pandemic. Now, let's examine death rates by age split between age categories.
<br/><br/>
```{r}
chi_covid_stats %>% 
  rowwise() %>% 
...
|:------------:|:---------:|:------:|:------:|:-------:|
| Total Deaths |    6024   | 2443.1 | 1951.5 |   4754.7|
|  Proportion  |     -     |   41%  |   32%  |   79%   |
...
* Phase 1B: January 25, 2021
  + Over 65 years old, most essential workers, shelters & prisons
...
The regression table shows the coefficients for each demographic of interest. Each has a statistically significant and negative value indicating and average reduction in deaths: 0.06 fewer per day for Black Chicagoans, 0.07 fewer for Latinx, and 0.18 for 60 & over. Note that the chart shows the same data in weekly totals to aid visual clarity.

Interestingly, the downward slope is relatively smooth across five month period on either side of the Phase 1B rollout on January 25. This indicates that the downward trend started earlier. Let's try the same thing, but looking at the timeframe around Phase 1A when the vaccine was made available to frontline healthcare workers.
...
Where $i$ represents a given day during Phase 1A, the dependent variable $Y$ represents deaths among people 60 and older, $\alpha$ represents the constant, $\tau$ represents the magnitude effect of vaccine availability $V$, $\beta$ represents the magnitude effect of belonging to a racial demographic $X$, and $\epsilon$ represents the error term.
<br/><br/>
```{r}
...
stargazer(rdd1,
          ...
          dep.var.labels = c("Deaths 60 and Over"),
          covariate.labels = c("Vax Available (Binary)", "Black", "Vax (Binary) x Black", "Latinx", "Vax (Binary) x Latinx"),
          align = TRUE,
          header = FALSE,
          type = "html",
          digits = 2,
          notes.align = "l",
          notes = c("dataset::CDPH", "rdd() multivariate"),
          notes.append = TRUE,
          omit.stat=c("f"))
```
<br/><br/>
Model 1 estimates the causal effect $\tau$ of the initial vaccine deployment in Phase 1A for the general population 60 and older is 3.16 fewer covid deaths on any given day between Nov. 1, 2020 and Feb. 28, 2021. Model 2 estimates the causal effect for Black residents age 60 and older as 4.7 fewer covid deaths a day, and Model 3 estimates 0.285 fewer for Latinx residents.

One important caveat in this final regression table is worth noting. Since the data is measuring proportions of populations within each demographic category and not individual subjects, there is some amount of colinearity. In other words, there is overlap between the dependent variable $Y$ and dependent variables $X$ since they are both death counts among age and racial groups. Ideally, I would have data on individual subjects who could be identified as either below or above 60 *and* belonging to a particular racial group. Instead, this dataset reports proportions of covid deaths among different demographic categories. The best way to think about the interaction variables $V*X$ is to consider how the proportion of covid deaths among a particular racial group contributes to or influences the proportion of covid deaths for people 60 and over.
